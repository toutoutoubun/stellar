  ...
  if (!activeResult) {
    return (
      <div className="flex items-center justify-center h-full" style={{ color: "var(--color-text-tertiary)" }}>
        <p className="text-sm">テキスト分析結果がありません</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 px-6 pt-5 pb-3">
        ...
        <div className="flex items-center gap-4 text-xs" style={{ color: "var(--color-text-secondary)" }}>
          <span>変数: <strong style={{ color: "var(--color-text-primary)" }}>{varName}</strong></span>
          <span>合計トークン: <strong style={{ color: "var(--color-text-primary)" }}>{activeResult.totalTokens.toLocaleString()}</strong></span>
          <span>異なり語数: <strong style={{ color: "var(--color-text-primary)" }}>{activeResult.uniqueTokens.toLocaleString()}</strong></span>
        </div>
      </div>
...
const WordsTab: React.FC<WordsTabProps> = memo(({ result, stopwords, swInput, setSwInput, addStopword, removeStopword, reanalyze, reanalyzing }) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold" style={{ color: "var(--color-text-primary)" }}>ワードクラウド</h3>
      ...
      <div style={{ backgroundColor: "var(--color-bg-secondary)", borderRadius: "var(--radius-lg)", border: "1px solid var(--color-border-primary)", padding: "16px" }}>
        <h3 className="text-sm font-semibold mb-3" style={{ color: "var(--color-text-primary)" }}>除外語の管理</h3>
    ...
    <div className="flex flex-col gap-4">
      ...
      {commLegend.length > 0 && (
        <div style={{ backgroundColor: "var(--color-bg-secondary)", borderRadius: "var(--radius-lg)", border: "1px solid var(--color-border-primary)", padding: "12px 16px" }}>
          <h4 className="text-xs font-semibold mb-2" style={{ color: "var(--color-text-secondary)" }}>コミュニティ</h4>
...
const StatsTab: React.FC<{
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {bigrams.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold mb-3" style={{ color: "var(--color-text-primary)" }}>Top バイグラム</h3>
          <div style={{ border: "1px solid var(--color-border-primary)", borderRadius: "var(--radius-md)", overflow: "hidden" }}>
            <table className="w-full text-xs" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: "var(--color-bg-secondary)" }}>
                  <th className="px-3 py-2 text-left font-medium" style={{ color: "var(--color-text-secondary)", borderBottom: "1px solid var(--color-border-primary)" }}>順位</th>
                  <th className="px-3 py-2 text-left font-medium" style={{ color: "var(--color-text-secondary)", borderBottom: "1px solid var(--color-border-primary)" }}>ペア</th>
                  <th className="px-3 py-2 text-left font-medium" style={{ color: "var(--color-text-secondary)", borderBottom: "1px solid var(--color-border-primary)" }}>平均頻度</th>
                </tr>
              </thead>
              <tbody>
                {bigrams.map((b, i) => (
      ...
      {perDocData.length > 1 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold" style={{ color: "var(--color-text-primary)" }}>テキスト別トークン数</h3>
