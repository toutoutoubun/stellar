    ...
    return (
      <div className="flex items-center justify-center h-full" style={{ color: "var(--color-text-tertiary)" }}>
        <div className="text-center">
          <p className="text-sm font-medium mb-1">ネットワークデータがありません</p>
          <p className="text-xs">十分な共起ペアが検出されませんでした</p>
        </div>
      </div>
    );
  }
        ...
        {tooltipData && (
          ...
          >
            ...
            <div className="flex flex-col gap-0.5 text-xs" style={{ color: "var(--color-text-secondary)" }}>
              <span>次数: {tooltipData.node.degree}</span>
              <span>媒介中心性: {fmt(tooltipData.node.betweenness)}</span>
              <span>近接中心性: {fmt(tooltipData.node.closeness)}</span>
              <span>コミュニティ: {tooltipData.node.community + 1}</span>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="flex flex-col">
      ...
      <div className="px-4 py-3" style={{ borderTop: "1px solid var(--color-border-primary)" }}>
        <div className="p-3" style={{ backgroundColor: "color-mix(in srgb, var(--color-accent-info) 6%, var(--color-bg-primary))", borderRadius: "var(--radius-md)", border: "1px solid color-mix(in srgb, var(--color-accent-info) 20%, var(--color-border-primary))" }}>
          <p className="text-xs font-semibold mb-1" style={{ color: "var(--color-text-primary)" }}>解釈</p>
