...
function deterministicJitter(seed: string, axis: "x" | "y", amplitude: number): number {
  let hash = axis === "x" ? 2166136261 : 16777619;
  for (let i = 0; i < seed.length; i++) {
    hash ^= seed.charCodeAt(i);
    hash = Math.imul(hash, 16777619);
  }
  return (((hash >>> 0) / 0xffffffff) - 0.5) * amplitude;
}
  ...
  useEffect(() => {
    ...
    inputNodes.forEach((node, i) => {
      if (!pos.has(node.id)) {
        ...
        pos.set(node.id, {
          x: cx + radius * Math.cos(angle) + deterministicJitter(node.id, "x", 30),
          y: cy + radius * Math.sin(angle) + deterministicJitter(node.id, "y", 30),
          vx: 0,
          vy: 0,
        });
      }
...
export const ForceGraph: React.FC<ForceGraphProps> = ({
   ...
   *
   * graphData は render 中に副作用を持たないよう純粋に作る。
   */
  const graphData = useMemo(() => {
    ...
    const positioned = (nodes as GraphNodeExtended[]).map((node, i) => {
      ...
      return {
        ...node,
        x: radius * Math.cos(angle) + deterministicJitter(node.id, "x", 20),
        y: radius * Math.sin(angle) + deterministicJitter(node.id, "y", 20),
  ...
  const emitNodePositions = useCallback(() => {
    if (!onNodePositionsChange) return;
    onNodePositionsChange(graphData.nodes);
